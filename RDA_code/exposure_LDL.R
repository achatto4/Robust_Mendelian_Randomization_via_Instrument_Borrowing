...
GWAS_FILE="./data/GWAS/lipid_GWAS/without_UKB_LDL_INV_EUR_HRC_1KGP3_others_ALL.meta.singlevar.results"
RSID_FILE="./data/GWAS/lipid_GWAS/selected_snp_ldl_n.txt"
OUTPUT_FILE="filtered_HDL_results_with_header_base_LDL.txt"
snp_vector <- readLines(RSID_FILE)[-1]

exposure_dat_LDL <- read_exposure_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_LDL_results_with_header_base_LDL.txt",
  sep = "\t",
  snp_col = "rsID",
...
outcome_dat_HDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_HDL_results_with_header_base_LDL.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_logTG <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_logTG_results_with_header_base_LDL.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
n_exp <- max(unique(exposure_dat_LDL$samplesize.exposure))
...
for (trait1_name in names(outcome_traits)) {
  ...
  for (count in seq_along(aux_traits)) {
    ...
    result <- harmonize_and_evaluate(
      exposure_data  = exposure_dat_LDL,
...
write.csv(final_results , file = "../../results/RDA_results/final_results_ldl.csv", row.names = FALSE)
