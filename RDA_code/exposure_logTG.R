...
GWAS_FILE="./data/GWAS/lipid_GWAS/without_UKB_logTG_INV_EUR_HRC_1KGP3_others_ALL.meta.singlevar.results"
RSID_FILE="./data/GWAS/lipid_GWAS/selected_snp_logTG_n.txt"
OUTPUT_FILE="filtered_HDL_results_with_header_base_logTG.txt"
snp_vector <- readLines(RSID_FILE)[-1]

exposure_dat_logTG <- read_exposure_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_logTG_results_with_header_base_logTG.txt",
  sep = "\t",
  snp_col = "rsID",
...
outcome_dat_HDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_HDL_results_with_header_base_logTG.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_LDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_LDL_results_with_header_base_logTG.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_nonHDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_nonHDL_results_with_header_base_LDL.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_TC <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_TC_results_with_header_base_LDL.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
n_exp <- max(unique(exposure_dat_logTG$samplesize.exposure))
...
outcome_traits <- list(
  astm_mvp = list(
    ...
    aux = list(
      ...
      dbp     = outcome_dat_dbp,
      Diastolic_Mean_INT_mvp = outcome_dat_Diastolic_Mean_INT_mvp
    )
  ),
  t2d_mvp = list(
...
for (trait1_name in names(outcome_traits)) {
  ...
  for (count in seq_along(aux_traits)) {
    ...
    result <- harmonize_and_evaluate(
      exposure_data  = exposure_dat_logTG,
...
write.csv(final_results , file = "../../results/RDA_results/final_results_logTG.csv", row.names = FALSE)
