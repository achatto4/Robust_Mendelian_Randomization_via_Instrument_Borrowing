...
RSID_FILE="./data/GWAS/vitD_GWAS/selected_snp_vitD_n1.txt"
snp_vector <- readLines(RSID_FILE)[-1]

exposure_dat_vitD <- read_exposure_data(
  filename            = "./data/GWAS/vitD_GWAS/Revezetal2020_25OHD.gz",
  sep                 = " ",
  snp_col             = "SNP",
  beta_col            = "BETA",
  se_col              = "SE",
...
outcome_dat_HDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_HDL_results_with_header_base_vitD.txt",
  sep = "\t",
  snp_col = "rsID",
...
outcome_dat_logTG <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_logTG_results_with_header_base_vitD.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_nonHDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_nonHDL_results_with_header_base_vitD.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_TC <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_TC_results_with_header_base_vitD.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
n_exp <- max(unique(exposure_dat_vitD$samplesize.exposure))
...
outcome_traits <- list(
  astm_mvp = list(
    ...
    aux = list(
      ...
      Trig_Mean_INT_mvp      = outcome_dat_Trig_Mean_INT_mvp,
      eGFR_Mean_INT_mvp      = outcome_dat_eGFR_Mean_INT_mvp
    )
  ),
  t2d_mvp = list(
...
for (trait1_name in names(outcome_traits)) {
  ...
  for (count in seq_along(aux_traits)) {
    ...
    result <- harmonize_and_evaluate(
      exposure_data  = exposure_dat_vitD,
...
write.csv(final_results , file = "../../results/RDA_results/final_results_vitD.csv", row.names = FALSE)
