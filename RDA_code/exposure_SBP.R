...
RSID_FILE="./data/GWAS/BP_GWAS/selected_snp_sbp_n.txt"
snp_vector <- readLines(RSID_FILE)[-1]

exposure_dat_SBP <- read_exposure_data(
...
exposure_dat_SBP <- exposure_dat_SBP %>%
...
outcome_dat_LDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_LDL_results_with_header_base_SBP.txt",
  sep = "\t",
  snp_col = "rsID",
...
outcome_dat_HDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_HDL_results_with_header_base_SBP.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_logTG <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_logTG_results_with_header_base_SBP.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_nonHDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_nonHDL_results_with_header_base_SBP.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_TC <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_TC_results_with_header_base_SBP.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
n_exp <- max(unique(exposure_dat_SBP$samplesize.exposure))
...
for (trait1_name in names(outcome_traits)) {
  ...
  for (count in seq_along(aux_traits)) {
    ...
    result <- harmonize_and_evaluate(
      exposure_data  = exposure_dat_SBP,
...
write.csv(final_results , file = "../../results/RDA_results/final_results_sbp.csv", row.names = FALSE)
