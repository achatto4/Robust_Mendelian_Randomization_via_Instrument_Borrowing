...
RSID_FILE="./data/GWAS/MrcML_sumstats/plink2_clump_results/selected_snp_eversmok.txt"
snp_vector <- readLines(RSID_FILE)[-1]

exposure_dat_eversmok <- read_exposure_data(
  filename = "./data/GWAS/MrcML_sumstats/summary_txt/eversmok.sumstats.txt",
  sep = "\t",
  snp_col = "rs_id",
  beta_col = "beta",
...
exposure_dat_eversmok <- exposure_dat_eversmok %>%
...
outcome_dat_LDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_LDL_results_with_header_base_eversmok.txt",
  sep = "\t",
  snp_col = "rsID",
...
outcome_dat_HDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_HDL_results_with_header_base_eversmok.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_logTG <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_logTG_results_with_header_base_eversmok.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_nonHDL <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_nonHDL_results_with_header_base_eversmok.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
outcome_dat_TC <- read_outcome_data(
  filename = "./data/GWAS/lipid_GWAS/filtered_TC_results_with_header_base_eversmok.txt",
  sep = "\t",
  snp_col = "rsID",
  beta_col = "EFFECT_SIZE",
  se_col = "SE",
...
n_exp <- max(unique(exposure_dat_eversmok$samplesize.exposure))

outcome_traits <- list(
  astm_mvp = list(
    data = outcome_dat_astm_mvp,
    aux = list(
      tia_mvp = outcome_dat_TIA_mvp,
      PE_mvp  = outcome_dat_PE_mvp,
      HDLC_Mean_INT_mvp = outcome_dat_HDLC_Mean_INT_mvp
    )
  ),
  t2d_mvp = list(
  ...
  cad_mvp = list(
    ...
    aux = list(
      CCD_mvp = outcome_dat_CCD_mvp,
      MI_mvp  = outcome_dat_MI_mvp,
      PVD_mvp = outcome_dat_PVD_mvp
    )
  ),
  stk_mvp = list(
    data = outcome_dat_stk_mvp,
    aux = list(
      PHD_mvp = outcome_dat_PHD_mvp,
      PE_mvp  = outcome_dat_PE_mvp,
      tia_mvp = outcome_dat_TIA_mvp
    )
  )
...
for (trait1_name in names(outcome_traits)) {
  ...
  for (count in seq_along(aux_traits)) {
    ...
    result <- harmonize_and_evaluate(
      exposure_data  = exposure_dat_eversmok,
...
write.csv(final_results , file = "../../results/RDA_results/final_results_eversmok.csv", row.names = FALSE)
