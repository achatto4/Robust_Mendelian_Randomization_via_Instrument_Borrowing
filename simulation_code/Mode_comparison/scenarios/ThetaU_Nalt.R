...
library(penalized)
library(ks)
# if (!requireNamespace("MRcML", quietly = TRUE)) {
...
theta_alt = theta_alt_vec[temp[7]]

pthr = 5e-8 # p-value threshold for instrument selection
NxNy_ratio = 2 # Ratio of sample sizes for X and Y
NxNy_alt_ratio = NxNy_alt_ratio_vec[temp[8]] # Ratio of sample sizes for X and Y
...
for (repind in 1:N_rep){
  ...
  if (numIV>2){
    ...
    if (repind == 1) {
      ...
      cat("BI: Absolute CoHeterogeneity Q value:", abs(CoHetQ$Q_corr_matrix)[1, 2], "\n")
    }
  ...
  if (repind%%5==0){
    ...
    save(est, file = paste0("../../../results/simulation_results/mode_comp/ThetaU_Nalt_est_theta", 
                            theta, 
                            "_thetaU", 
                            thetaU, 
                            "_N", 
