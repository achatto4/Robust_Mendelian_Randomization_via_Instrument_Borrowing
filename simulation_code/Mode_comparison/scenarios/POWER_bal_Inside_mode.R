# Summary level simulations with balanced pleiotropy and InSIDE assumption satisfied (POWER setting)
rm(list = ls())
...
for (repind in seq_len(N_rep)) {
  ...
  if (numIV > 2) {
    ...
    if (repind == 1) {
      ...
      cat("BI: Absolute CoHeterogeneity Q value:", abs(cohet$Q_corr_matrix)[1, 2], "\n")
    }
...
base_name <- paste0(
  "BI_POWER_theta", theta, "_thetaAlt", theta_alt, "_phi1_thetaU", thetaU,
