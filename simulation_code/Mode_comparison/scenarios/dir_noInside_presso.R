...
for(repind in 1:N_rep) {
  ...
  alpha[ind2]       <- rnorm(length(ind2), 0.008, sqrt(sigma2y_td))
  alpha[ind3]       <- rnorm(length(ind3), 0.008, sqrt(sigma2y))
  ...
  if(repind %% 5 == 0) save(est,
                            file = sprintf(
                              "../../../results/simulation_results/presso_comp/sim_MRpressos_DN_beta_p_se_theta%s_U%s_N%s_invalid%s_overlap%s.rda",
