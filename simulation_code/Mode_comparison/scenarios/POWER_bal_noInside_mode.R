# Summary level simulations with balanced pleiotropy but InSIDE assumption not satisfied (POWER setting)
rm(list = ls())
...
# Power setting for primary effect evaluation
theta_vec <- c(-0.1, 0.1)
phi_val <- 1

# Secondary trait effect grid (varied in power setting)
theta_alt_vec <- c(-0.2, -0.1, 0, 0.1, 0.2)
thetaU_alt <- 0.3

thetaUvec <- c(0.3)
Nvec <- c(5e4, 8e4, 1e5, 2e5)
NxNy_alt_ratio_vec <- c(0.2, 0.5, 1, 2, 5)
prop_invalid_vec <- c(0.1, 0.3, 0.5, 0.7)
overlap_vec <- c(0, 0.25, 0.5, 0.75, 1)
alpha_levels <- c(0.005, 0.01, 0.05, 0.1)

# Argument format: theta_idx theta_alt_idx thetaU_idx N_idx prop_invalid_idx phi_idx overlap_idx alt_ratio_idx
args <- as.integer(commandArgs(trailingOnly = TRUE))
if (length(args) < 8 || any(is.na(args))) {
  stop("Expected 8 integer args: theta_idx theta_alt_idx thetaU_idx N_idx prop_invalid_idx phi_idx overlap_idx alt_ratio_idx")
}

theta <- theta_vec[args[1]]
theta_alt <- theta_alt_vec[args[2]]
thetaU <- thetaUx <- thetaUvec[args[3]]
N <- Nvec[args[4]]
prop_invalid <- prop_invalid_vec[args[5]]
overlap <- overlap_vec[args[7]]
NxNy_alt_ratio <- NxNy_alt_ratio_vec[args[8]]
...
for (repind in seq_len(N_rep)) {
  ...
  if (numIV > 2) {
    ...
    if (repind == 1) {
      ...
      cat("BN: Absolute CoHeterogeneity Q value:", abs(cohet$Q_corr_matrix)[1, 2], "\n")
    }
...
power_summary <- do.call(rbind, lapply(alpha_levels, function(a) {
  data.frame(
    scenario = "BN",
    theta = theta,
    theta_alt = theta_alt,
    ...
    power_mrmode = mean(mode_p < a, na.rm = TRUE),
    power_ib_mode = mean(ib_mode_p < a, na.rm = TRUE),
    thetaU = thetaU,
    N = N,
    NxNy_alt_ratio = NxNy_alt_ratio,
    prop_invalid = prop_invalid,
...
results_dir <- "../../../results/simulation_results/mode_comp_power"
dir.create(results_dir, recursive = TRUE, showWarnings = FALSE)

base_name <- paste0(
  "BN_POWER_theta", theta, "_thetaAlt", theta_alt, "_phi1_thetaU", thetaU,
  "_N", format(N, scientific = FALSE),
  "_NxNyAlt", NxNy_alt_ratio,
...
save(
  est,
  power_summary,
  file = file.path(results_dir, paste0(base_name, ".rda"))
)

write.csv(
  power_summary,
  file = file.path(results_dir, paste0(base_name, "_power_summary.csv")),
  row.names = FALSE
)
