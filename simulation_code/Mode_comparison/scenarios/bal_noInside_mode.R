# Summary level simulations with balanced pleiotropy but InSIDE assumption not satisfied
rm(list = ls())
library(data.table)
library(dplyr)
library(MASS)
...
phi_val_vec = exp(seq(log(0.1), log(10), length.out = 25))
...
phi_val = phi_val_vec[temp[5]]
overlap = overlap_vec[temp[6]]
...
for (repind in 1:N_rep){
  ...
  if (numIV>2){
    ...
    if (repind == 1) {
      ...
      cat("BN: Absolute CoHeterogeneity Q value:", abs(CoHetQ$Q_corr_matrix)[1, 2], "\n")
    }
    ...
    est[repind,3+3] = res$StdError
    est[repind,3+5] = T1-T0
    rm(res)
  ...
  if (repind%%5==0){
    ...
    save(est, file = paste0("../../../results/simulation_results/mode_comp/BN_est_theta", 
                            theta, 
                            "_thetaU", 
                            thetaU, 
                            "_N", 
